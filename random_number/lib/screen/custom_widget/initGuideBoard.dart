...
//OPEN SOURCE FROM
//https://github.com/escamoteur/super_Board/edit/master/lib/super_Board.dart

////////////////////////////////////////////////////////////////////////////////////////////////////
/// Super flexible Board class that allows you to show any content
/// inside a Board in the overlay of the screen.
///
class CustomNextFuncBoard {

  /// Allows to accedd the closebutton for UI Testing
  static Key closeButtonKey = const Key("CloseButtonKey");

  /// Signals if the Board is visible at the moment
  bool isOpen = false;
  ///
  /// The content of the Board
  final Widget content;
  ///
  /// The direcion in which the Board should open
  BoardDirection popupDirection;
  ///
  /// optional handler that gets called when the Board is closed
  ...
  ///
  /// The minium padding from the Board to the screen limits
  final double minimumOutSidePadding;
  ///
  /// If [snapsFarAwayVertically== true] the bigger free space above or below the target will be
  /// covered completely by the Board. All other dimension or position constraints get overwritten
  ...
  /// [top], [right], [bottom], [left] position the Board absolute relative to the whole screen
  double top, right, bottom, left;
  ///
  /// A Board can have none, an inside or an outside close icon
  final ShowCloseButton showCloseButton;
  ///
  /// [hasShadow] defines if the Board should have a shadow
  ...
  ///
  /// The backgroundcolor of the Board
  final Color backgroundColor;
  /// The color of the rest of the overlay surrounding the Board.
  /// typically a translucent color.
  final Color outsideBackgroundColor;
  ///
  /// By default touching the surrounding of the Board closes the Board.
  ...
  ///
  /// Let's you pass a key to the Boards cotainer for UI Testing
  final Key BoardContainerKey;
  ...
  CustomNextFuncBoard({
    this.BoardContainerKey,
    @required this.content, // The contents of the Board.
  ...
  ///
  /// Removes the Board from the overlay
  void close() {
    if (onClose != null) {
      onClose();
    }
  ...
  void show(BuildContext targetContext, double posX, double posY) {
...
//    targetCenter = renderBox.localToGlobal(renderBox.size.center(Offset.zero), ancestor: overlay);
    ...
    if (snapsFarAwayVertically) {
      ...
      if (targetCenter.dy > overlay.size.center(Offset.zero).dy) {
        ...
        popupDirection = BoardDirection.down;
        bottom = 0.0;
      }
      ...
      if (targetCenter.dx < overlay.size.center(Offset.zero).dx) {
        popupDirection = BoardDirection.right;
        right = 0.0;
      } else {
        popupDirection = BoardDirection.left;
        left = 0.0;
      }
    }
...
class _PopupBallonLayoutDelegate extends SingleChildLayoutDelegate {
  BoardDirection _popupDirection;
...
class _BubbleShape extends ShapeBorder {
  ...
  final BoardDirection popupDirection;
